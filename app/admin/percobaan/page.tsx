  ...
  return (
    <div className="w-full min-h-screen bg-[#F8F9FA] flex flex-col md:flex-row font-sans text-gray-900">
      ...
      <button 
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-[#005C43] text-white rounded-full shadow-xl flex items-center justify-center hover:-translate-y-1 transition-transform z-50"
      >
        {isChatOpen ? <X className="w-6 h-6" /> : <MessageCircle className="w-6 h-6" />}
      </button>

      {isChatOpen && (
        <div className="fixed bottom-24 right-6 w-80 bg-white border border-gray-100 rounded-2xl shadow-2xl z-50 flex flex-col overflow-hidden">
          <div className="bg-[#005C43] p-4 text-white">
            <h3 className="font-bold text-sm flex items-center gap-2"><MessageCircle className="w-4 h-4" /> Admin Chat</h3>
            <p className="text-[10px] text-emerald-100 mt-0.5">Diskusikan masalah teknis</p>
          </div>
          <div className="p-4 h-64 overflow-y-auto bg-gray-50 custom-scrollbar flex flex-col gap-3">
            <div className="bg-emerald-100 text-emerald-800 p-2 rounded-lg rounded-tl-none text-xs w-[85%]">Halo Admin! Ada yang bisa saya bantu terkait sistem?</div>
          </div>
          <div className="p-3 border-t border-gray-100 bg-white flex gap-2">
            <input 
              ...
              className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#005C43]"
            />
            <button className="bg-[#005C43] text-white p-2 rounded-lg hover:bg-[#004733] transition-colors"><Send className="w-4 h-4" /></button>
          </div>
        </div>
      )}
