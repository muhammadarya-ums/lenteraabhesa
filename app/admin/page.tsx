...
interface SusunKataItem {
  id: string;
  clue_indonesia: string;
  kalimat_benar: string;
}
...
interface TebakGambarItem {
  ...
  cultural_fact: string | null;
  pengecoh_1: string | null;
  pengecoh_2: string | null;
...
const [selectedYear, setSelectedYear] = useState(new Date().getFullYear())
  ...
  const [analyticsGame, setAnalyticsGame] = useState({totalSesi:0})

  // chat & notification states
  const [isChatOpen, setIsChatOpen] = useState(false)
  const [currentUserId, setCurrentUserId] = useState<string | null>(null)
  const [contacts, setContacts] = useState<ProfilAdmin[]>([])
  const [selectedContact, setSelectedContact] = useState<ProfilAdmin | null>(null)
  ...
  const [contohAlos, setContohAlos] = useState('')
  const [contohSedang, setContohSedang] = useState('')
  const [contohKasar, setContohKasar] = useState('')

  const [artiContohAlos, setArtiContohAlos] = useState('')
  const [artiContohSedang, setArtiContohSedang] = useState('')
  const [artiContohKasar, setArtiContohKasar] = useState('')
  ...
  const [tgForm, setTgForm] = useState({ image_url: '', question: '', jawaban_benar: '', hint: '', explanation: '', cultural_fact: '', pengecoh_1: '', pengecoh_2: '', pengecoh_3: '' })
  ...
  const [audioContohAlos, setAudioContohAlos] = useState<File | null>(null)
  const [audioContohSedang, setAudioContohSedang] = useState<File | null>(null)
  const [audioContohKasar, setAudioContohKasar] = useState<File | null>(null)
...
// INITIAL LOAD
  useEffect(() => {
    const initDashboard = async () => {
      try {
        ...
        const { count: countPermainan } = await supabase.from('permainan').select('*', { count: 'exact', head: true })
        setTotalGame(countPermainan || 0)
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      ...
      if (!error && data) setContacts(data as ProfilAdmin[])
    }
    fetchContacts()
  }, [currentUserId])
                              ...
                              <div key={order.id} className={`p-5 border rounded-2xl flex flex-col lg:flex-row justify-between gap-6 hover:shadow-sm transition-all ${order.status === 'dibatalkan' ? 'bg-gray-100/50 border-gray-200 opacity-80' : 'bg-gray-50/50 border-gray-100 hover:border-gray-200'}`}>
                                ...
                                <div className="flex flex-row sm:flex-col justify-between lg:justify-center items-end gap-4 shrink-0 border-t sm:border-t-0 pt-4 sm:pt-0 border-gray-200">
                                  <div className="text-left sm:text-right">
                                    ...
                                    <p className="text-lg font-black text-[#005C43]">Rp {order.total_harga.toLocaleString('id-ID')}</p>
                                  </div>
